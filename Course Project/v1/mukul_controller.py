    ...
    def port_modify_handler(self, ev):
        ...
        self.logger.info(
            ...
            % (
                ...
                port_attr.state,
                port_attr.curr,
                port_attr.advertised,
                port_attr.supported,
                port_attr.peer,
                port_attr.curr_speed,
                port_attr.max_speed,
            )
        )
        if port_attr.state == 1:
            ...
            if removed_link is not None:
                (
                    shortest_path_hubs,
                    shortest_path_node,
                ) = self.topo_shape.find_shortest_path(removed_link.src.dpid)
                print(
                    "\t\tNew shortest_path_hubs: {0}\n\t\tNew shortest_path_node: {1}".format(
                        shortest_path_hubs, shortest_path_node
                    )
                )
        ...
        while s_count > len(visited):
            ...
            for l in self.find_links_with_src(s_temp):
                ...
                if l.dst.dpid in shortest_path_hubs:
                    # Find the minimum o
                    if (
                        shortest_path_hubs[l.src.dpid] + 1
                        < shortest_path_hubs[l.dst.dpid]
                    ):
                        shortest_path_hubs[l.dst.dpid] = (
                            shortest_path_hubs[l.src.dpid] + 1
                        )
                    ...
                    if verbose == 1:
                        print(
                            "\t\tdst dpid found in shortest_path. Count: "
                            + str(shortest_path_hubs[l.dst.dpid])
                        )
                elif (
                    l.src.dpid in shortest_path_hubs
                    and l.dst.dpid not in shortest_path_hubs
                ):
                    if verbose == 1:
    ...
    def find_path_from_topo(self, src_dpid, dst_dpid, shortest_path_node):
        ...
        while now_node != src_dpid:
            ...
            if last_node != None:
                ...
                if l is None:
                    print(
                        "Link between {0} and {1} was not found in topo.".format(
                            now_node, last_node
                        )
                    )
                else:
