    ...
    def handler_switch_event(self, ev):
        ...
        for switch in self.switches:
            ...
            for host in self.hosts:
                match = parser.OFPMatch(eth_dst=host.mac, eth_type=0x086DD)
                ...
                if dest_id < len(self.switches):
                    for link in self.links:
                        ...
                        if (
                            ...
                            out_port = int(link["src"]["port_no"])
                else:
                    out_port = int(host.port.port_no)
                actions = [parser.OFPActionOutput(out_port)]
...
# app_manager.require_app("ryu.app.rest_firewall")
